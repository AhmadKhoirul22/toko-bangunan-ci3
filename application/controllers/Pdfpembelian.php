<?php 
class Pdfpembelian extends CI_Controller{
	...
	public function cetakpembelian(){
		...
		$this->pembelian($tanggal_1,$tanggal_2);
	}

	public function pembelian($tanggal_1,$tanggal_2){
		error_reporting(0); // AGAR ERROR MASALAH VERSI PHP TIDAK MUNCUL
        $pdf = new FPDF('L', 'mm','Letter');
        $pdf->AddPage();
        $pdf->SetFont('Arial','B',16);
        $pdf->Cell(0,7,'Laporan Pembelian',0,1,'C');
        ...
        $pdf->SetFont('Arial','B',10);
        $pdf->Cell(215,6,'Total Pembelian',1,0,'L');
	...
	public function status(){
        ...
        $pdf->AddPage();
        $pdf->SetFont('Arial','B',16);
        $pdf->Cell(0,7,'Laporan Data Piutang',0,1,'C');
        $pdf->Cell(0,7,tanggal_indo($tanggal_1).' sampai '.tanggal_indo($tanggal_2),0,1,'C');
        $pdf->Cell(10,7,'',0,1);
        $pdf->SetFont('Arial','B',10);
        $pdf->Cell(20,6,'No',1,0,'C');
        ...
        foreach ($data as $data){
            $no++;
            $pdf->Cell(20,6,$no,1,0, 'C');
            $pdf->Cell(35,6,$data->kode_pembelian,1,0);
            $pdf->Cell(35,6,$data->nama_supplier,1,0);
            $pdf->Cell(40,6,tanggal_indo($data->tanggal),1,0);
	...
	public function cetakcicilan(){
		...
		$this->load->view('pembelian/cetakcicilan',$data);
	}
	...
	public function cetakpembayaran(){
		...
		$this->load->view('pembelian/cetakpembayaran',$data);
	}
}
?>
