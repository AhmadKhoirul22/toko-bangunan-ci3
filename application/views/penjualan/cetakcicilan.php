<?php
        ...
        $pdf->SetFont('Arial','B',10);
		if($status == 0){
			$pdf->Cell(245,6,'Total sudah bayar',1,0,'L');
			$pdf->Cell(35,6,'Rp '.number_format($total, 2),1,1,'R');
			$pdf->Cell(245,6,'Total sisa tagihan',1,0,'L');
			$pdf->Cell(35,6,'Rp '.number_format($total_utang, 2),1,1,'R');
		} else if($status == 1){
		$pdf->Cell(245,6,'Total uang dari tagihan',1,0,'L');
        $pdf->Cell(35,6,'Rp '.number_format($total, 2),1,1,'R');
		} else if($status == 2){
		// $pdf->Cell(245,6,'Total tagihan',1,0,'L');
        // $pdf->Cell(40,6,'Rp '.number_format($tagihan, 2),1,1,'R');
		$pdf->Cell(245,6,'Total sudah bayar',1,0,'L');
        $pdf->Cell(35,6,'Rp '.number_format($total, 2),1,1,'R');
		$pdf->Cell(245,6,'Total sisa tagihan',1,0,'L');
        $pdf->Cell(35,6,'Rp '.number_format($total_utang, 2),1,1,'R');
